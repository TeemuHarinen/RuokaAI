...
const FoodPlanTable = ({ foodPlan, total }) => {
  ...
  return (
    <div>
      <h1>Ruokasuunnitelma</h1>
      <table border="1">
        <thead>
          <tr>
            <th>Resepti</th>
            <th>Reseptin kuvaus</th>
            <th>Kalorit</th>
            <th>Proteiini (g)</th>
            <th>Hiilihydraatit (g)</th>
            <th>Rasva (g)</th>
            <th> Ainesosat </th>
            <th> Valmistusohjeet </th>
          </tr>
        </thead>
        <tbody>
          <tr>
            ...
            <td>{foodPlan.breakfast.ingredients}</td>
            <td>{foodPlan.breakfast.instructions}</td>
          </tr>
          <tr>
            <td>Lounas</td>
            ...
            <td>{foodPlan.lunch.ingredients}</td>
            <td>{foodPlan.lunch.instructions}</td>
          </tr>
          <tr>
            <td>Päivällinen</td>
            ...
            <td>{foodPlan.dinner.ingredients}</td>
            <td>{foodPlan.dinner.instructions}</td>
          </tr>
          <tr>
            <td>Iltapala</td>
            ...
            <td>{foodPlan.evening_meal.ingredients}</td>
            <td>{foodPlan.evening_meal.instructions}</td>
          </tr>
          <tr>
            <td>Yhteenlaskettu</td>
            <td></td>
...
const Home = () => {
  ...
  const addNewAllergen = () => {
    if (newAllergen.trim() !== "") {
      // prevent empty allegens from being added
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:3001/api/food-plan", {
        calories,
        weightGoal,
  ...
  return (
    <div style={{ containerStyle }}>
      <form onSubmit={handleSubmit} style={formStyle}>
        <h1 style={headingStyle}>RuokaAI</h1>
        <h5 style={headingStyle}> Created by Teemu Harinen</h5>
        <h4 style={{ headingStyle, fontStyle: "italic" }}>
          Täytä tietosi alapuolelle saadaksesi päivän ruokasuunnitelman
        </h4>
        ...
        <label style={labelStyle}>
          ...
          >
            ...
            <option value="1.9">
              Extra active (very hard exercise/physical job & exercise)
            </option>
          </select>
        </label>
        <label style={labelStyle}>
      ...
      {foodPlan && (
        <FoodPlanTable
          style={tableStyle}
          foodPlan={foodPlan.food_plan}
          total={foodPlan.total}
        />
      )}
    </div>
  );
